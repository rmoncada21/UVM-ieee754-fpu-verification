...
# #-------------------------
# $(RUNNER_EXE): $(RUNNER_C) $(SF_LIBRARY) | $(RUN_BIN)
...
runner_run_default_case: $(RUNNER_EXE) | $(LOGS_RUN_C)
	$(TF_GEN) -level $(TF_LEVEL) -seed $(TF_SEED) -rnear_even f32_mul \
	  ...
	  | tee $(LOGS_RUN_C)/runner_mul_rne_$(FECHA).log
...
runner_run_matrix: $(RUNNER_EXE) | $(LOGS_RUN_C) 
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
	    	...
	    	| tee $(LOGS_RUN_C)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
	  done; \
	done; \
	for op in $(OPS_CMP); do \
	    ...
	    | tee $(LOGS_RUN_C)/$@_$${op}_rne_$(FECHA).log || exit 1; \
	done
	...
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			...
			| valgrind --tool=memcheck --leak-check=full --error-exitcode=1 \
				./$< $$rm $$op \
			| tee $(LOGS_RUN_VAL)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
		done ; \
	done ; \
	for op in $(OPS_CMP); do \
		echo -e "\n========== runner $$op =========="; \
		...
		| valgrind --tool=memcheck --leak-check=full --error-exitcode=1 \
			./$< $$rm $$op \
		| tee $(LOGS_RUN_VAL)/$@_$${op}_$(FECHA).log || exit 1; \
	done

#### massif
# TODO: agregar --log-file al valgrind y
# | tee $(LOGS_RUN_VAL)/$@_$${rm/-/}_$${op}_$(FECHA).log || exit 1;
	...
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			...
			| valgrind --tool=massif --error-exitcode=1 \
				--massif-out-file=$(LOGS_RUN_VAL)/$@_$${op}_$${rm/-/}_$(FECHA).log \
				./$< $$rm $$op \
			| tee $(LOGS_RUN_VAL)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
		done ; \
	done ; \
	...
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			...
			| valgrind --tool=callgrind --error-exitcode=1 \
				--callgrind-out-file=$(LOGS_RUN_VAL)/$@_$${op}_$${rm/-/}_$(FECHA).log \
				./$< $$rm $$op \
			| tee $(LOGS_RUN_VAL)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
		done ; \
	done ; \
...
runner_run_asan: $(RUNNER_ASAN) | $(LOGS_RUN_SAN)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			...
			$(TF_GEN) -level $(TF_LEVEL) -seed $(TF_SEED) $$rm $$op \
			|	./$< $$rm $$op \
			| tee $(LOGS_RUN_SAN)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
		done ; \
	done ; \
	for op in $(OPS_CMP); do \
		echo -e "\n========== runner $$op =========="; \
		$(TF_GEN) -level $(TF_LEVEL) -seed $(TF_SEED) $$rm $$op \
		|	./$< $$rm $$op \
		| tee $(LOGS_RUN_SAN)/$@_$${op}_$(FECHA).log || exit 1; \
	done
