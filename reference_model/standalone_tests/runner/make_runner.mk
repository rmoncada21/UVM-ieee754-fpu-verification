...
# #-------------------------
# $(RUNNER_EXE): $(RUNNER_C) $(SF_LIBRARY) | $(REF_BIN)
...
runner_run_default_case: $(RUNNER_EXE) | $(LOGS_RUNNER)
...
# 18 tests en total
runner_run_matrix: $(RUNNER_EXE) | $(LOGS_RUNNER) 
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
	    	...
	    	| tee $(LOGS_RUNNER)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
	  done; \
	done; \
	for op in $(OPS_CMP); do \
	    ...
	    | tee $(LOGS_RUNNER)/$@_$${op}_rne_$(FECHA).log || exit 1; \
	done
...
runner_memcheck: $(RUNNER_EXE) | $(LOGS_VALGRIND)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			rm=$${pair##*:}; \
			echo -e "\n========== runner $$rm $$op =========="; \
	...
	for op in $(OPS_CMP); do \
		...
		| valgrind --tool=memcheck --leak-check=full --error-exitcode=1 \
			./$< $$rm $$op \
		| tee $(LOGS_VALGRIND)/$@_$${op}_$(FECHA).log || exit 1; \
	done

#### massif
# TODO: agregar --log-file al valgrind y
# | tee $(LOGS_VALGRIND)/$@_$${rm/-/}_$${op}_$(FECHA).log || exit 1;
runner_massif: $(RUNNER_EXE) | $(LOGS_VALGRIND)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			rm=$${pair##*:}; \
			echo -e "\n========== runner $$rm $$op =========="; \
			...
			| valgrind --tool=massif --error-exitcode=1 \
				--massif-out-file=$(LOGS_VALGRIND)/$@_$${op}_$${rm/-/}_$(FECHA).log \
				./$< $$rm $$op \
			| tee $(LOGS_VALGRIND)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
		done ; \
	done ; \
...
#### callgrind
# TODO: agregar --log-file al valgrind y
runner_callgrind: $(RUNNER_EXE) | $(LOGS_VALGRIND)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			rm=$${pair##*:}; \
			echo -e "\n========== runner $$rm $$op =========="; \
			...
			| valgrind --tool=callgrind --error-exitcode=1 \
				--callgrind-out-file=$(LOGS_VALGRIND)/$@_$${op}_$${rm/-/}_$(FECHA).log \
				./$< $$rm $$op \
			| tee $(LOGS_VALGRIND)/$@_$${op}_$${rm/-/}_$(FECHA).log || exit 1; \
		done ; \
	done ; \
...
#### address
$(RUNNER_ASAN): FORCE $(SF_LIBRARY_A) | $(REF_BIN)
	$(CLANG) $(CLFLAGS) -fsanitize=address $(REF_C) $(RUNNER_C) $(SF_LIBRARY_A) $(INCLUDES) -o $@

runner_compile_asan: $(RUNNER_ASAN)

runner_run_asan: $(RUNNER_ASAN) | $(LOGS_SAN)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			rm=$${pair##*:}; \
			echo -e "\n========== runner $$rm $$op =========="; \
...
#### memory
$(RUNNER_MSAN): FORCE $(SF_LIBRARY_A) | $(REF_BIN)
	$(CLANG) $(CLFLAGS) -fsanitize=memory $(REF_C) $(RUNNER_C) $(SF_LIBRARY_A) $(INCLUDES) -o $@

runner_compile_msan: $(RUNNER_MSAN)

runner_run_msan: $(RUNNER_MSAN) | $(LOGS_SAN)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			rm=$${pair##*:}; \
			echo -e "\n========== runner $$rm $$op =========="; \
...
#### undefined
$(RUNNER_UBSAN): FORCE $(SF_LIBRARY_A) | $(REF_BIN)
	$(CLANG) $(CLFLAGS) -fsanitize=undefined $(REF_C) $(RUNNER_C) $(SF_LIBRARY_A) $(INCLUDES) -o $@

runner_compile_ubsan: $(RUNNER_UBSAN)

runner_run_ubsan: $(RUNNER_UBSAN) | $(LOGS_SAN)
	for op in $(OPS_ARITH); do \
		for pair in $(ROUND_MODE_PAIR); do \
			rm=$${pair##*:}; \
			echo -e "\n========== runner $$rm $$op =========="; \
