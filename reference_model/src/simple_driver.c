...
int main(void) {
    ...
    dpi_fpu_reference(0, 0x3F800000, 0x40000000, 0, 0, &round_mode, &flags);
    printf("FADD 1.0+2.0  = 0x%08X flags=0x%02X\n", round_mode, flags);

    /* FMUL min-normal(2^-126) * 0.5  -> subnormal 2^-127 = 0x00400000 */
    dpi_fpu_reference(2, 0x00800000, 0x3F000000, 0, 0, &round_mode, &flags);
    printf("FMUL subnormal= 0x%08X flags=0x%02X\n", round_mode, flags);

    /* FLT 1.0 < 2.0  -> 1 */
    dpi_fpu_reference(6, 0x3F800000, 0x40000000, 0, 0, &round_mode, &flags);
